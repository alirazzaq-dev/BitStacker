  ...
  const fetchContractDetails = async () => {
    if (provider && account) {
      try {
        ...
        const totalBlackHases = Number(hashes.blackHash) + Number(hashes.vipBlackHash);
        const totalBlueHases = Number(hashes.blueHash) + Number(hashes.vipBlueHash);
      
        setBalances({ total: totalBalance, totalBlack, totalBlue, ...balance });
        setHashes({ total: totalHashes, totalBlackHases, totalBlueHases, ...hashes });
      } catch (e) {
        console.error(e);
      }
    }
  ...
  return (
    <div className="flex space-x-5">
      ...
      <div className="bg-[#121212] flex flex-col justify-between rounded-3xl px-5 py-3 max-w-[442px] w-full ">
        ...
        <div className=" w-[270px] mx-auto my-4 flex items-center justify-between">
        </div>


            {donut === DONUT.NFTS ? `${balances?.total} Tokens` : `${hashes?.total} Terrahash`}
            <Doughnut width={250} height={250} data={donut === DONUT.NFTS ? dataNFTs : dataHashes} />
              ...
              <form className="flex flex-col items-center mt-5 space-y-5">
                <div className="w-full relative border border-[#595959] rounded-[50px] py-4 px-6 my-1">
                  <input
                    ...
                    onChange={(e) => handleUpdatedContactInfo(e.target.value, "bitcoin")}
                ...
                <div className="w-full relative border border-[#595959] rounded-[50px] py-4 px-6 my-1">
                  <input
                    ...
                    onChange={(e) => handleUpdatedContactInfo(e.target.value, "email")}
                    className="w-full bg-transparent outline-none font-medium text-lg"
                  />
                  <p className="absolute -top-4 left-7  font-light text-base px-1 bg-[#121212]">
