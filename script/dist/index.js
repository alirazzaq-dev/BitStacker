...
workbook.xlsx.readFile(filePath)
    ...
    for (let i = 0; i < allAddress.length; i++) {
        ...
        const userMonthlyHashes = JSON.parse(JSON.stringify(userRowUpdated.values)).slice(2, 33);
        ...
        const monthlyRewardData = JSON.parse(JSON.stringify(DBTCRRowUpdated.values)).slice(2, 33);
...
const getTotalHashesOfTheDay = async () => {
    const provider = new ethers_1.ethers.providers.JsonRpcProvider("https://rinkeby.infura.io/v3/d9fb7fe853c94891811d279e8055defe");
    const contract = new ethers_1.Contract(addresses.BitStackerNFT, abis.BitStackerNFT, provider);
